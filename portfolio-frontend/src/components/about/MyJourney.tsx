    ...
    <section id="journey" className="scroll-mt-24 py-24">
      <Container className="max-w-5xl">
        ...
        <motion.div
          ...
          whileInView="visible"
          viewport={VIEWPORT}
          className="relative mt-16 border-l border-white/10 pl-8"
        >
          {entries.map((entry) => (
            <motion.div key={entry.title} variants={staggerItem} className="relative mb-10 last:mb-0">
              <span className="absolute -left-[2.2rem] top-2 flex h-5 w-5 items-center justify-center rounded-full border-4 border-[var(--color-bg-base)] bg-[var(--color-accent-blue)]" />
              <GlassCard className="group rounded-3xl p-6 transition-all duration-300 hover:-translate-y-1 hover:border-[var(--color-accent-blue)]/40">
                ...
                <h3 className="mt-3 text-2xl font-bold">{entry.title}</h3>
                <p className="mt-4 leading-8 text-[var(--color-text-secondary)]">{entry.body}</p>
              </GlassCard>
            </motion.div>
          ))}
        </motion.div>
